 ...
 * Returns operation loads to perform on the document and a list of original
...
cot.core.Document.prototype.transform_ = function(op, contextOrDiff, missing) {
  ...
  for (var i = 0; i < diff.length; i++) {
    ...
    if (goog.isDefAndNotNull(op['load']) && goog.isDefAndNotNull(opx['load'])) {
      if (goog.DEBUG) {
...
cot.net.Connections = function() {
   ...
   * @private
   */
  this.collectionInterval_ = 1000 * 60 * 5;
...
cot.net.Connections.prototype.popUpdates = function(documentId, siteId) {
  ...
  if (this.isConnected(documentId, siteId)) {
    ...
    tuple[0] = connectedSite.packages;
    tuple[1] = connectedSite.sitesChanged ?
...
cot.ops.string.txRemoveRemove = function(op, against) {
  ...
  var otherVal = otherLoad['value'];
  var otherLen = otherVal.length;
  var from = Math.max(otherAt - at, 0);
  var to = Math.min(otherAt + otherLen - at, len);
  if (from <= to) {
    if (from == 0) {
      if (to == len) {
        return null;
    ...
    val = val.substring(0, from) + val.substring(to);
    return cot.ops.string.remove(at, val);
  }
  return load;
};
